...
*
	...
	贴到leetcode上运行会失败，这个和695应该是一个数据结构，但这里是byte ，那里是int
*/
...
func num(grid [][]byte, i, j int) int {
...
func isVisited(grid [][]byte, i, j int) bool {
	return grid[i][j] > 1
}

func isNotInGrid(grid [][]byte, i, j int) bool {
